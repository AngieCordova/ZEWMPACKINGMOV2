sap.ui.define([
    ...
    return Controller.extend("AvocadoProyecto.AvocadoProyecto.modules.inventarioContenedoresEWM.controller.inventarioContenedoresEWM", {
        ...
        handleFilter: function() {
            ...
            */
            var othat = this;
            var oBinding = othat.getView().byId("idReportTable").getBinding("items");
            var oFilter;
            var aFilters = [];
        ...
        handleCleanFilter: function() {
            ...
            var oBinding = this.getView().byId("idReportTable").getBinding("items");
            oBinding.filter([]);
            this.scope.totalRows = oBinding.aIndices.length;
            var oModelScope = new sap.ui.model.json.JSONModel(this.scope);
            this.getView().setModel(oModelScope, "scope");
        ...
        onGetCSV: function() {
            var oBinding = this.getView().byId("idReportTable");
            var data = [];
            var tmpData = [];
            for (let index = 0; oBinding.getColumns()[index]; index++) {
                tmpData.push(oBinding.getColumns()[index].getHeader().getProperty('text'));
            }
            data.push(tmpData);
